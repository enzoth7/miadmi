...
export default function EstimacionGeneralPage() {
...
useEffect(() => {
  ...
  // Generar 12 filas donde "Saldo mes pasado" es encadenado (punto 8)
  ...
  const filas = useMemo(() => {
    ...
    return arr;
  }, [isActive, saldoInicial, totalIngresos, totalEgresos, ahorro]);

  // KPIs (punto 9)
  const kpiAhorroTotal12 = useMemo(() => {
    if (!filas.length) return 0;
    return Math.round(filas.reduce((a, f) => a + (Number.isFinite(f.ahorro) ? f.ahorro : 0), 0));
  }, [filas]);

  const saldoFinal12 = filas.length ? filas[filas.length - 1].saldoFinal : 0;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-semibold text-white">Estimacion general</h1>
          <p className="text-white/80">
            Carga ingresos y egresos generales, defini un ahorro mensual y mira la proyeccion en una tabla de 12 meses.
          </p>
          {!isActive ? (
            <p className="mt-2 text-sm text-amber-100/90">Esta estimacion esta desactivada; sus valores no se incluyen en Inicio hasta volver a activarla.</p>
          ) : null}
        </div>
      ...
      <section className="rounded-2xl border border-sky-100 bg-sky-50 p-6 text-gray-900 shadow">
        <h2 className="text-lg md:text-xl font-semibold mb-4">Proyeccion (12 meses)</h2>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50 text-gray-700">
              <tr>
                ...
                {filas.map((f, idx) => (
                  <th key={f.idx} className="px-3 py-2 text-right">{monthLabels[idx]}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y">
              {[
                { key: "saldoMesPasado", label: "Saldo mes pasado" },
                ...
                <tr key={row.key} className="hover:bg-gray-50/60">
                  ...
                  {filas.map((f) => {
                    const val = f[row.key];
                    const extra = row.key === "saldoFinal" ? "font-semibold" : "";
                    const neg = row.key === "saldoFinal" && val < 0 ? "text-rose-600" : "";
                    return (
                      <td key={f.idx} className={["px-3 py-2 text-right tabular-nums", extra, neg].join(" ")}>{formatUYU(val)}</td>
                    );
                  })}
                </tr>
              ))}
