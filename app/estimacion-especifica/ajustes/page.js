...
export default function AjustesEstimacionEspecificaPage() {
  ...
  const resolvedEgresos = useMemo(() => {
    return EXPENSE_CATEGORIES.map((cat) => {
      ...
      if (cat.id === "prestamos") {
        baseValue = prestamosSeries;
      } else if (cat.id === "tarjetas") {
        baseValue = tarjetasSeries;
      }
      const overrides = projection.egresos?.[cat.id];
      const values = Array.from({ length: 12 }, (_, idx) => {
        const raw = overrides?.[idx];
        if (raw !== undefined && raw !== "") return n(raw);
        // Si baseValue es un array, usar el valor del índice correspondiente
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      try {
        ...
        setPrestamosSeries(prestamosSchedule.series);
        setTarjetasSeries(tarjetasSchedule.series);
  ...
  useEffect(() => {
    ...
    const handleStorage = (event) => {
      if (event?.key && event.key !== LS_ESTIMABLES) return;
  ...
  }, [session.supabase, session.userId, currentMonthKey]);
